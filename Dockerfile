...
# and `pip install eeharvest` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "eeharvest", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n eeharvest -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "eeharvest", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "eeharvest", "YOUR_CONSOLE_SCRIPT"]
